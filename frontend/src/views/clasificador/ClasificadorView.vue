    ...
    >
      <div class="bg-white text-black rounded-xl p-6 w-full max-w-md shadow-xl">
        <h2 class="text-lg font-bold mb-4">
          Ingresar cantidad reciclada para {{ usuarioSeleccionado.name }}
        </h2>
        <p>Se te daran un total de 10 puntos por libra de materia reciclada.</p>
        <label class="block mb-2">Cantidad en libras:</label>
        <input
          v-model="cantidadReciclada"
          type="number"
...
const guardarCantidad = () => {
  guardarPuntos(usuarioSeleccionado.value.id, cantidadReciclada.value)
  mensaje.value = `Cantidad de ${cantidadReciclada.value} lbs registrada para ${usuarioSeleccionado.value.name}.`
  setTimeout(() => {
    mensaje.value = ''
  }, 2000)
...
//   // si usas filtro por backend, envía filtros a la API.
//   // aquí lo hacemos localmente.
// }
