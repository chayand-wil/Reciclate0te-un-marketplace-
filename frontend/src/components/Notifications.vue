  ...
  <div ref="menuRef" class="relative">
    ...
    >
      ...
      >
        ...
        <g
          id="Icons-56/notification_outline_56"
          stroke="none"
          stroke-width="1"
          fill="none"
          fill-rule="evenodd"
    ...
    <div
      v-if="open"
      class="absolute mt-6 right-0 mt-2 w-64 rounded-xl bg-white/10 backdrop-blur-sm shadow-xl text-white p-4 z-50"
    >
      <template v-if="user.name">
        <div class="mt-4">
          <h2 class="text-xs font-semibold mb-2">Notificaciones</h2>
          <ul class="space-y-2">
            <li v-for="notifi in notifications" :key="index" class="bg-gray-800 p-2 rounded-md">
              <h4 class="text-sm">{{ notifi.motivo_notificacion.slug }}</h4>
              <p class="text-sm">{{ notifi.motivo_notificacion.nombre }}</p>
              <span class="text-xs text-gray-400">{{ formatFecha(notifi?.created_at) }}</span>
            </li>
          </ul>
        </div>
      </template>
...
onMounted(async () => {
  try {
    ...
    if (res.data.rol.slug !== 'reutilizador') {
      ...
      user.value = res.data
      const res2 = await api.get(`/get_notifications/${user.value.id}`)
